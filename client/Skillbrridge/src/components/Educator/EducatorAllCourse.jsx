...
const EducatorAllCourse = () => {
  ...
  const [rawCourses, setRawCourses] = useState([]); // debug: store raw API response
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (coursesResponse.success && coursesResponse.courses) {
        ...
        // store raw response for visual debug
        ...
        const transformedCourses = allCourses.map(course => {
          ...
          // Normalize current user id (handle different shapes)
          const currentUserId = user?._id || user?.id || null;

          // Normalize educator id from course (handle populated object or raw id)
          let educatorId = null;
          if (course.educator) {
            if (typeof course.educator === 'string') educatorId = course.educator;
            else if (course.educator._id) educatorId = course.educator._id;
          }

          const isOwn = currentUserId && educatorId && educatorId.toString() === currentUserId.toString();
  ...
  const EmptyState = () => (
    ...
    >
      ...
      <Link
        ...
        className="inline-flex items-center space-x-2 px-6 py-3 bg-linear-to-r from-blue-600 to-purple-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200"
      >
  ...
  const CourseCard = ({ course }) => (
    <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl overflow-hidden hover:border-slate-600/50 transition-all duration-300 group">
      ...
      <div className="relative h-36 bg-linear-to-r from-blue-600 to-purple-600">
        {course.thumbnail ? (
          <img 
      ...
      <div className="p-4">
        ...
        <div className="flex space-x-2 pt-3 border-t border-slate-700">
          ...
          <Link
            ...
            className="flex-1 px-3 py-2 bg-linear-to-r from-blue-600 to-purple-600 text-white font-medium rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-200 flex items-center justify-center space-x-1 cursor-pointer hover:scale-105 text-sm"
      ...
      <div className="container mx-auto px-6 lg:px-8 pb-16">
        ...
        >
          ...
          <div className="flex items-center space-x-3">
            ...
            <Link
              to="/educator/add-course"
              className="px-6 py-3 bg-linear-to-r from-blue-600 to-purple-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 flex items-center space-x-2"
            >
              <BookOpen className="w-5 h-5" />
        ...
        {/* VISIBLE DEBUG: raw API response */}
        {/* {rawCourses && rawCourses.length > 0 && (
          <div className="mb-6 max-w-5xl mx-auto p-4 bg-slate-900/60 border border-slate-700 rounded-lg text-sm text-slate-300">
            <div className="font-medium text-white mb-2">Debug: Raw API response (first 5 courses)</div>
            <div className="overflow-auto text-xs">
              {rawCourses.slice(0,5).map((c, i) => (
                <div key={c._id || i} className="mb-2 p-2 bg-slate-800/30 rounded">
                  <div><strong>id:</strong> {String(c._id)}</div>
                  <div><strong>title:</strong> {c.title}</div>
                  <div><strong>educator:</strong> {c.educator ? (c.educator._id || String(c.educator)) : 'null'}</div>
                  <div><strong>isPublished:</strong> {String(c.isPublished)}</div>
                </div>
              ))}
            </div>
          </div>
